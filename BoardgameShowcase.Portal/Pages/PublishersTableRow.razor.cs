...
{
    public partial class PublishersTableRow
    {
        private const int MAX_BOARDGAME_COUNT = 3;

        [Inject] private IBoardgameService BoardgameService { get; set; } = default!;

        [Parameter, EditorRequired] public Publisher Publisher { get; set; } = default!;
        ...
        {
            Boardgames = await BoardgameService.GetByPublisherAsync(Publisher);
        }
    }
}
